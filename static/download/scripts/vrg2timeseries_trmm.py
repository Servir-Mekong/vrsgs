...
class vrg2ts(object):
    ...
    def getPrecip(self):
        ...
        for i in range(n_iter):
            ...
            if self.product == 'GSMAP':
                ...
                ftpDir = '/VRG/TRMM/DAILY/MM/'.format(now.year,now.month)
                ftpFile = 'MM_3B42_daily.{0}.{1:02d}.{2:02d}.7.nc'.format(now.year,now.month,now.day)
                ftpPath = ftpDir + ftpFile
    ...
    def makeTimeSeries(self):
        ...
        for i in range(n_iter):
            now = t1 + datetime.timedelta(i)
            ncs = datadir + 'MM_3B42_daily.{0}.{1:02d}.{2:02d}.7.nc'.format(now.year,now.month,now.day)
            print(ncs)
...
def main():
    ...
    parser.add_argument('--product','-p',choices=['TRMM','GSMAP'],required=True,
    ...
    if args.product not in ['TRMM','GSMAP']:
