...
class vrg2ts(object):
    ...
    def getPrecip(self):
        ...
        for i in range(n_iter):
            ...
            if self.product == 'GSMAP':
                ...
                ftpDir = '/VRG/IMERG/DAILY/{0}/{1:02d}/MM/'.format(now.year,now.month)
                ftpFile = 'MM_3B-DAY-E.MS.MRG.3IMERG.{0}{1:02d}{2:02d}-S000000-E235959.V04.nc'.format(now.year,now.month,now.day)
                ftpPath = ftpDir + ftpFile
    ...
    def makeTimeSeries(self):
        ...
        for i in range(n_iter):
            now = t1 + datetime.timedelta(i)
            ncs = datadir + 'MM_3B-DAY-E.MS.MRG.3IMERG.{0}{1:02d}{2:02d}-S000000-E235959.V04.nc'.format(now.year,now.month,now.day)
            print(ncs)
