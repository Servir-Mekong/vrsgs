...
1) create a time series from GSMAP

  $ python vrg2timeseries.py -p GSMAP -i 2017-01-01 -e 2017-01-31

2) create a time series from GSMAP over specific region

  $ python vrg2timeseries.py -p GSMAP -i 2017-01-01 -e 2017-01-31 -s ./test.shp

3) create a time series from GSMAP in specific directory

  $ python vrg2timeseries.py -p GSMAP -i 2017-01-01 -e 2017-01-31 -d ./Users/user/Desktop/
  
4) create a time series from GSMAP over specific region an into specific directory

  $ python vrg2timeseries.py -p GSMAP -i 2017-01-01 -e 2017-01-31 -s ./test.shp -d ./Users/user/Desktop/
...
class vrg2ts(object):
    ...
    def getPrecip(self):
        ...
        for i in range(n_iter):
            ...
            if self.product == 'GGSMAP':
                ...
                ftpFile = 'MM_gsmap_gauge.{0}{1:02d}{2:02d}.0.1d.daily.00Z-23Z.v7.1002.0.nc'.format(now.year,now.month,now.day)
    ...
    def makeTimeSeries(self):
        ...
        for i in range(n_iter):
            now = t1 + datetime.timedelta(i)
            ncs = datadir + 'MM_gsmap_gauge.{0}{1:02d}{2:02d}.0.1d.daily.00Z-23Z.v7.1002.0.nc'.format(now.year,now.month,now.day)
            print(ncs)
...
def main():
    ...
    parser.add_argument('--product','-p',choices=['GSMAP','GGSMAP'],required=True,
    ...
    if args.product not in ['GSMAP','GGSMAP']:
