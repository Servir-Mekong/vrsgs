...
1) create a time series from CMORPH

  $ python vrg2timeseries.py -p CMORPH -i 2017-01-01 -e 2017-01-31

2) create a time series from CMORPH over specific region

  $ python vrg2timeseries.py -p CMORPH -i 2017-01-01 -e 2017-01-31 -s ./test.shp

3) create a time series from CMORPH in specific directory

  $ python vrg2timeseries.py -p CMORPH -i 2017-01-01 -e 2017-01-31 -d ./Users/user/Desktop/
  
4) create a time series from CMORPH over specific region an into specific directory

  $ python vrg2timeseries_cmorph_MM.py -p CMORPH -i 2017-01-01 -e 2017-01-31 -s ./test.shp -d ./Users/user/Desktop/
...
class vrg2ts(object):
    ...
    def getPrecip(self):
        ...
        for i in range(n_iter):
            ...
            if self.product == 'GGSMAP':
                ftpDir = '/VRG/GSMAP/DAILY_STD/MM/{0}/{1:02d}/'.format(now.year,now.month)
                ftpFile = 'MM_gsmap_gauge.{0}{1:02d}{2:02d}.0.1d.daily.00Z-23Z.v7.0001.0.nc'.format(now.year,now.month,now.day)
                ftpPath = ftpDir + ftpFile
            else:
                ftpDir = '/VRG/CMORPH/DAILY_V1.0/MM/{0}/{1:02d}/'.format(now.year,now.month)
                ftpFile = 'MM_CMORPH_V1.0_RAW_0.25deg-DLY_00Z_{0}{1:02d}{2:02d}.nc'.format(now.year,now.month,now.day)
                ftpPath = ftpDir + ftpFile
    ...
    def makeTimeSeries(self):
        ...
        for i in range(n_iter):
            now = t1 + datetime.timedelta(i)
            ncs = datadir + 'MM_CMORPH_V1.0_RAW_0.25deg-DLY_00Z_{0}{1:02d}{2:02d}.nc'.format(now.year,now.month,now.day)
            print(ncs)
            ...
            try:
                ...
                if self.product == 'GGSMAP':
                    ...
                    pr_var = nc.variables['comb']
                    precip  = pr_var[0,0,:,:]
                    noData = pr_var.missing_value
                
                #precip = np.swapaxes(precip,0,25)
...
def main():
    ...
    parser.add_argument('--product','-p',choices=['CMORPH','GGSMAP'],required=True,
    ...
    if args.product not in ['CMORPH','GGSMAP']:
        print('Product type not recognized: {0}\nPlease select either TRMM or GSMAP',format(args.product))
        sys.exit()
